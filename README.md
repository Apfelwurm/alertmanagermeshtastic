...
# Node failure handling (optional, defaults shown below) [Example backoff progression with defaults: 30s → 60s → 120s → 240s → 300s (capped)]
failure_threshold = 3    # Number of failures before node is temporarily skipped
base_backoff_time = 30   # Initial backoff time in seconds
...
The system now supports sending alerts to multiple Meshtastic nodes with resilient delivery. You can configure this in two ways:
...
The system includes robust failure handling for improved reliability:

- **Per-node queuing**: Each node has its own message queue, so failures on one node don't affect others
- **Automatic retry**: Failed messages are automatically retried with exponential backoff
- **Failure isolation**: Nodes that repeatedly fail are temporarily skipped to prevent blocking other nodes. All failure handling behavior can be customized via config.toml (see example above)
